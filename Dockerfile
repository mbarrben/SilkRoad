...
# RUN rm -rf ./*
# RUN cp /root/swift-5.8-android-x86_64-24-sdk/usr/lib/x86_64-linux-android/*.so ./
# RUN cp /root/swift-5.8-android-x86_64-24-sdk/usr/lib/*.so ./

WORKDIR /root/lib/arm64-v8a
RUN rm -rf ./*
RUN cp /root/swift-5.8-android-24-sdk/usr/lib/aarch64-linux-android/*.so ./
# RUN cp /root/swift-5.8-android-24-sdk/usr/lib/*.so ./
...
#
RUN /usr/bin/termux-install libc/libcurl/libcurl_8.4.0 libcurl.so libcurl.so
